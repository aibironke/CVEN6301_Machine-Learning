# -*- coding: utf-8 -*-
"""
Created on Wed Apr 26 22:20:06 2023
...
sm = pd.read_csv('SanMarcos.csv', skiprows=range(29), usecols=lambda x: x not in [1, 2, 5])


# drop columns 1, 2 & 5
sm = sm.drop(sm.columns[[0, 1, 4]], axis=1)
...
sm.set_index('datetime', inplace=True)
...
sm = sm.loc[start_date:end_date]

# Split the data into training, validation, and test sets
train_data = sm.iloc[:int(len(sm)*0.4)] #selects the first 40% of the data (rows) for the training set
val_data = sm.iloc[int(len(sm)*0.4):int(len(sm)*0.7)] # selects the rows between the first 40% and 70% for validation
test_data = sm.iloc[int(len(sm)*0.7):]#selects the remaining 30% of the data for the test set
...
def create_sequences(data, seq_length):
    X = [] #empty list to append inputs into
    y = [] #empty list to append output to
...
X_train, y_train = create_sequences(train_scaled, seq_length)#assigns normalized input data"trained_scaled" along "seq_length to output variables "X_train & y_train"
...
model.add(LSTM(units=64, input_shape=(seq_length, 1))) #LSTM layer added w/64 LSTM units and input shape "seq_lenth" and the 1 indicates that there is only one feature (i.e., Discharge) in the input. 
